...
class BookDownloadWorker @AssistedInject constructor(
    ...
    override suspend fun doWork(): Result = withContext(Dispatchers.IO) {
        ...
        // Only delete the existing DB record if the output file doesn't already exist
        // (i.e. we're starting fresh, not replacing). This prevents wiping a good
        // download record before we know the new download will succeed.
        val existingFile = File(outputDir, "${ratingKey}_part0.${
            (inputData.getString(KEY_PART_KEYS) ?: "").split(",")
                .firstOrNull()?.substringAfterLast('.', "m4b") ?: "m4b"
        }")
        ...
        for ((index, partKey) in partKeys.withIndex()) {
            ...
            try {
                ...
                if (body == null) {
                    return@withContext fail("Server sent an empty response body")
                }
                ...
                // Calculate byte cap based on downloadHours setting.
                // We use the content-length and total book duration to find bytes-per-ms,
                // then multiply by desired hours. This works regardless of bitrate.
                // Falls back to downloading the full file if duration is unknown.
                val maxBytes: Long = if (contentLength > 0 && durationMs > 0) {
                    val bytesPerMs = contentLength.toDouble() / durationMs
                    val desiredMs = sessionManager.downloadHours * 3600 * 1000L
                    ...
                    if (cap >= contentLength) {
                        ...
                        Log.i(TAG, "Capping download at ${sessionManager.downloadHours}h = $cap bytes of $contentLength total")
                        cap
                    }
                } else {
